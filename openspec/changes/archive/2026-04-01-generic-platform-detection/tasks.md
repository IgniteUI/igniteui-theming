...
- [x] 3.4 In the generic branch, add a "Sass Configuration" section that inspects `result.signals` for config file signals and emits framework-specific load path guidance (angular.json → stylePreprocessorOptions.includePaths, vite.config → css.preprocessorOptions.scss.loadPaths, next.config → sassOptions.loadPaths, fallback → general guidance)
