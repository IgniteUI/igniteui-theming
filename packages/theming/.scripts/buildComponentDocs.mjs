...
function parsePrimaryTokens(rawDescription) {
  ...
  for (let i = 0; i < contentLines.length; i++) {
    ...
    if (parsingTokens && line.startsWith("- `$")) {
      ...
      !parsingTokens ||
      (parsingTokens && primaryTokens.length > 0 && !line.startsWith("- `$"))
    ) {
      parsingTokens = false;
      if (line) {
...
async function extractComponentThemes() {
  ...
  const themeFunctions = data.filter(
    (item) =>
      item.context.type === "function" && item.context.name.endsWith("-theme"),
  );
  ...
  await mkdir("./dist/json/components", { recursive: true });
  await writeFile(
    "./dist/json/components/themes.json",
