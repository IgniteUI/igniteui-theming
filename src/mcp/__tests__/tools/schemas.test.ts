...
describe("colorSchema", () => {
	...
	describe("HSL/HSLA colors", () => {
		...
		it("accepts legacy comma-separated hsla()", () => {
			expect(colorSchema.safeParse("hsla(180, 50%, 50%, 0.5)").success).toBe(
				true,
			);
		...
		it("accepts modern hsl() with slash alpha", () => {
			expect(colorSchema.safeParse("hsl(180 50% 50% / 0.5)").success).toBe(
				true,
			);
			expect(colorSchema.safeParse("hsl(180 50% 50% / 50%)").success).toBe(
				true,
			);
		});
	...
	describe("HWB colors (CSS Color Level 4)", () => {
		...
		it("accepts hwb() with alpha", () => {
			expect(colorSchema.safeParse("hwb(180 20% 30% / 0.5)").success).toBe(
				true,
			);
			expect(colorSchema.safeParse("hwb(180 20% 30% / 50%)").success).toBe(
				true,
			);
		});
	...
	describe("LAB colors (CSS Color Level 4)", () => {
		...
		it("accepts lab() with alpha", () => {
			expect(colorSchema.safeParse("lab(50% 40 59.5 / 0.5)").success).toBe(
				true,
			);
			expect(colorSchema.safeParse("lab(50% 40 59.5 / 50%)").success).toBe(
				true,
			);
		});
	...
	describe("OKLAB colors (CSS Color Level 4)", () => {
		...
		it("accepts oklab() with alpha", () => {
			expect(colorSchema.safeParse("oklab(59% 0.1 0.1 / 0.5)").success).toBe(
				true,
			);
			expect(colorSchema.safeParse("oklab(59% 0.1 0.1 / 50%)").success).toBe(
				true,
			);
		});
	...
	describe("OKLCH colors (CSS Color Level 4)", () => {
		...
		it("accepts oklch() with alpha", () => {
			expect(colorSchema.safeParse("oklch(60% 0.15 50 / 0.5)").success).toBe(
				true,
			);
			expect(colorSchema.safeParse("oklch(60% 0.15 50 / 50%)").success).toBe(
				true,
			);
		});
		...
		it("accepts oklch() with hue angle units", () => {
			...
			expect(colorSchema.safeParse("oklch(60% 0.15 0.14turn)").success).toBe(
				true,
			);
		});
	});
	...
	describe("color() function (CSS Color Level 4)", () => {
		it("accepts color() with display-p3", () => {
			expect(colorSchema.safeParse("color(display-p3 1 0.5 0)").success).toBe(
				true,
			);
			expect(colorSchema.safeParse("color(display-p3 0 1 0)").success).toBe(
				true,
			);
		});
		...
		it("accepts color() with srgb", () => {
			...
			expect(colorSchema.safeParse("color(srgb 0.5 0.5 0.5)").success).toBe(
				true,
			);
		});
		...
		it("accepts color() with alpha", () => {
			expect(
				colorSchema.safeParse("color(display-p3 1 0.5 0 / 0.5)").success,
			).toBe(true);
			expect(colorSchema.safeParse("color(srgb 1 0 0 / 50%)").success).toBe(
				true,
			);
		});

		it("accepts color() with other color spaces", () => {
			expect(colorSchema.safeParse("color(srgb-linear 1 0 0)").success).toBe(
				true,
			);
			expect(colorSchema.safeParse("color(a98-rgb 1 0 0)").success).toBe(true);
			expect(colorSchema.safeParse("color(prophoto-rgb 1 0 0)").success).toBe(
				true,
			);
			expect(colorSchema.safeParse("color(rec2020 1 0 0)").success).toBe(true);
			expect(colorSchema.safeParse("color(xyz 0.5 0.5 0.5)").success).toBe(
				true,
			);
			expect(colorSchema.safeParse("color(xyz-d50 0.5 0.5 0.5)").success).toBe(
				true,
			);
			expect(colorSchema.safeParse("color(xyz-d65 0.5 0.5 0.5)").success).toBe(
				true,
			);
		});
	});
	...
	describe("edge cases", () => {
		...
		it("accepts calc() expressions inside color functions", () => {
			...
			expect(colorSchema.safeParse("rgb(calc(255 * 0.5) 0 0)").success).toBe(
				true,
			);
		});
	});
...
describe("layout schemas", () => {
	...
	it("rejects negative spacing values", () => {
		...
		expect(
			setSpacingSchema.safeParse({ spacing: 1, inline: -0.5 }).success,
		).toBe(false);
		expect(
			setSpacingSchema.safeParse({ spacing: 1, block: -0.25 }).success,
		).toBe(false);
	});

	it("enforces radiusFactor between 0 and 1", () => {
		expect(setRoundnessSchema.safeParse({ radiusFactor: 0 }).success).toBe(
			true,
		);
		expect(setRoundnessSchema.safeParse({ radiusFactor: 1 }).success).toBe(
			true,
		);
		expect(setRoundnessSchema.safeParse({ radiusFactor: 1.1 }).success).toBe(
			false,
		);
	});
});
...
describe("readResourceSchema", () => {
	it("accepts a valid URI string", () => {
		expect(
			readResourceSchema.safeParse({ uri: "theming://platforms" }).success,
		).toBe(true);
		expect(
			readResourceSchema.safeParse({ uri: "theming://presets/palettes/light" })
				.success,
		).toBe(true);
	});
