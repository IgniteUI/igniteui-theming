...
describe('CSS_VARIABLE_PATTERNS', () => {
  ...
  it('has correct scrollbar variables', () => {
    ...
    expect(CSS_VARIABLE_PATTERNS.scrollbar.thumbBackground).toBe('--ig-scrollbar-thumb-background');
    expect(CSS_VARIABLE_PATTERNS.scrollbar.trackBackground).toBe('--ig-scrollbar-track-background');
  });

  it('has correct patterns for color and elevation', () => {
    expect(CSS_VARIABLE_PATTERNS.colorPattern).toBe('--ig-{color}-{shade}');
    expect(CSS_VARIABLE_PATTERNS.elevationPattern).toBe('--ig-elevation-{level}');
  });
});

describe('VARIABLE_PATTERNS', () => {
  it('generates correct palette preset variable', () => {
    expect(VARIABLE_PATTERNS.palettePreset('light', 'material')).toBe('$light-material-palette');
    expect(VARIABLE_PATTERNS.palettePreset('dark', 'indigo')).toBe('$dark-indigo-palette');
  });

  it('generates correct schema variable', () => {
    expect(VARIABLE_PATTERNS.schema('light', 'material')).toBe('$light-material-schema');
    expect(VARIABLE_PATTERNS.schema('dark', 'fluent')).toBe('$dark-fluent-schema');
  });
  ...
  it('generates correct type scale variable', () => {
    expect(VARIABLE_PATTERNS.typeScale('material')).toBe('$material-type-scale');
    expect(VARIABLE_PATTERNS.typeScale('bootstrap')).toBe('$bootstrap-type-scale');
  });

  it('generates correct elevations variable', () => {
    expect(VARIABLE_PATTERNS.elevations('material')).toBe('$material-elevations');
    expect(VARIABLE_PATTERNS.elevations('indigo')).toBe('$indigo-elevations');
  });

  it('generates correct custom palette variable', () => {
    expect(VARIABLE_PATTERNS.customPalette('brand', 'light')).toBe('$brand-light-palette');
    expect(VARIABLE_PATTERNS.customPalette('company', 'dark')).toBe('$company-dark-palette');
  });
});
