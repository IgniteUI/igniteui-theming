...
describe("generatePaletteCss", () => {
	it("should generate CSS custom properties for a basic palette", async () => {
		...
		const colorValuePattern =
			/--ig-primary-500:\s*(hsl|hsla|rgb|rgba|color|#)[\w\d(),.\s%-]+;/;
	...
	it("should generate valid color values for all palette shades", async () => {
		...
		for (const shade of SHADE_LEVELS) {
			...
			expect(value, `--ig-primary-${shade} should be a valid color`).toMatch(
				colorPattern,
			);
		}
		...
		expect(
			contrastValue,
			"--ig-primary-500-contrast should have a value",
		).not.toBeNull();
		expect(
			contrastValue,
			"--ig-primary-500-contrast should be a valid color",
...
describe("generateComponentThemeCss", () => {
	...
	it("should generate CSS custom properties for an avatar theme", async () => {
		const { generateComponentThemeCss } = await import(
			"../../generators/css.js"
		);
