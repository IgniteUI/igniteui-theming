...
function collectAllColors(input: CreateCustomPaletteInput): {
  ...
  for (const group of CHROMATIC_COLOR_GROUPS) {
    ...
    if (definition) {
      collectFromDefinition(group, definition, ALL_COLOR_SHADES, colors, missingShades);
    }
  }

  // Collect gray colors
  if (input.gray) {
    collectFromDefinition('gray', input.gray, [...SHADE_LEVELS], colors, missingShades);
  }
...
function collectFromDefinition(
  ...
  if (definition.mode === 'shades') {
    ...
    if (definition.contrastOverrides) {
      for (const [shade, color] of Object.entries(definition.contrastOverrides)) {
...
export async function validateCustomPalette(
  ...
  if (input.gray?.mode === 'explicit') {
    await validateShadeProgression('gray', input.gray.shades, 'gray', variant, warnings);
  }
 ...
 */
export function formatCustomPaletteValidation(result: CustomPaletteValidationResult): string {
