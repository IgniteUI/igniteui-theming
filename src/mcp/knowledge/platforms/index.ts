...
export type DetectionSignal =
	| PackageDetectionSignal
	| ConfigFileDetectionSignal
		...
		for (const csproj of csprojFiles) {
			try {
				...
				if (
					...
					content.includes("Microsoft.NET.Sdk.Razor")
				) {
...
export function detectPlatformFromDependencies(
	...
	const significantPlatforms = sorted.filter(
		([, score]) => score >= ambiguousThreshold,
	);

	if (significantPlatforms.length > 1) {
		// Build alternatives array with their signals
		const alternatives: PlatformAlternative[] = significantPlatforms.map(
			([platform, score]) => {
				const platformSignals = signals.filter((s) => {
					...
					if (s.type === "config_file") {
						return configSignals.some(
							(cs) => cs.platform === platform && cs.file === s.file,
						);
					}
	...
	const detectedPackageSignal = signals.find(
		(s) =>
			s.type === "ignite_package" &&
			IGNITE_PACKAGE_PATTERNS[topPlatform]?.includes(s.package),
