...
export async function handleDetectPlatform(params: DetectPlatformParams) {
	...
	if (result.ambiguous && result.alternatives) {
		...
		text +=
			"The project appears to contain dependencies for multiple Ignite UI platforms. ";
		text +=
			"This might be a monorepo or a project transitioning between frameworks.\n\n";
		...
		if (result.confidence === "low") {
			text += "\n\n### Note\n\n";
			text +=
				"Detection confidence is **low**. This means no Ignite UI package was found, ";
			text +=
				"only framework packages. Please verify this is the correct platform before generating themes.";
		} else if (result.confidence === "medium") {
			text += "\n\n### Note\n\n";
			text +=
				"Detection confidence is **medium**. Consider verifying the platform if the generated ";
