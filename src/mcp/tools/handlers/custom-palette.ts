...
export async function handleCreateCustomPalette(
	...
	const colors: {
		...
		info:
			(params.info as ColorDefinition) ??
			({ mode: "shades", baseColor: preset.info } as ShadesBasedColor),
		success:
			(params.success as ColorDefinition) ??
			({ mode: "shades", baseColor: preset.success } as ShadesBasedColor),
		warn:
			(params.warn as ColorDefinition) ??
			({ mode: "shades", baseColor: preset.warn } as ShadesBasedColor),
		error:
			(params.error as ColorDefinition) ??
			({ mode: "shades", baseColor: preset.error } as ShadesBasedColor),
	};
...
async function handleCssOutput(
	...
	try {
		...
		if (shadesMode.length > 0) {
			responseParts.push(
				`**Using shades() function:** ${shadesMode.join(", ")}`,
			);
		}
		if (explicitMode.length > 0) {
			responseParts.push(
				`**Using explicit shades:** ${explicitMode.join(", ")}`,
			);
		}
...
function handleSassOutput(
	...
	const paletteName = params.name
		? toVariableName(params.name)
		: `custom-${variant}`;
