...
${availablePlatforms.length > 0 ? `Available platforms: ${availablePlatforms.join(', ')}.` : ''}`,
      ...
      if (selectorsEntry.webcomponents) {
        selectors = selectors.concat(buildSelectorList(selectorsEntry.webcomponents));
      }

      if (selectorsEntry.angular && selectorsEntry.webcomponents) {
        notes.push('Platform not specified; output includes Angular and Web Components selectors.');
      }
    }
...
function formatSelectorBlock(selectors: string[], declarations: string[]): string {
  const selectorLine = selectors.join(',\n');
  const lines = [selectorLine, '{', ...declarations.map((line) => `  ${line}`), '}'];
...
export async function handleSetSpacing(params: SetSpacingParams) {
  ...
  if (inline !== undefined) declarations.push(`--ig-spacing-inline: ${inline};`);
  if (block !== undefined) declarations.push(`--ig-spacing-block: ${block};`);

  const code = formatSelectorBlock(selectors, declarations);
  const description = `Set spacing in ${scopeLabel}.`;
  const guidance =
    output === 'sass' ? ['Sass note: pad() functions require @include spacing() once at root scope.'] : [];
...
export async function handleSetRoundness(params: SetRoundnessParams) {
  ...
  const guidance =
    output === 'sass' ? ['Sass note: border-radius() responds to --ig-radius-factor without extra mixins.'] : [];
