 ...
 */
export function generatePaletteCode(options: PaletteCodeOptions): PaletteCodeResult {
 ...
 */
export function generateTypographyCode(options: TypographyCodeOptions): string[] {
 ...
 * Returns an array of lines that can be joined or embedded in other code.
 */
export function generateElevationsCode(options: ElevationsCodeOptions): string[] {
  const indent = options.indent ?? '';
 ...
 * });
 */
export function generateCustomPaletteCode(options: CustomPaletteCodeOptions): string[] {
  const variant = options.variant ?? 'light';
  ...
  for (let i = 0; i < colorGroups.length; i++) {
    ...
    if (def.mode === 'shades') {
      ...
      for (let j = 0; j < shades.length; j++) {
        ...
        const contrast = def.contrastOverrides?.[shade as keyof typeof def.contrastOverrides];
